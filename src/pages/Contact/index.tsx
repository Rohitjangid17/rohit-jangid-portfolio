...
export default function Contact() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setStatus('loading')
    try {
      // TODO: wire this up to a real form service — e.g. Formspree, Resend, or EmailJS.
      // No backend is configured yet, so submissions are not actually sent.
      ...
      <section ref={formSectionRef} className="py-12 sm:py-16 border-t border-border scroll-mt-20">
        <Container>
          <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-14">
            ...
            <ContactForm status={status} fieldErrors={fieldErrors} onSubmit={handleSubmit} onReset={() => setStatus('idle')} />
          </div>
        </Container>
      </section>
    </>
...
function ContactHero({ onStart }: { onStart: () => void }) {
  const reduceMotion = useReducedMotion()
...
function SignalGraphic({ reduceMotion }: { reduceMotion: boolean }) {
  const nodes = [
    { x: 55, y: 60 },
    { x: 260, y: 40 },
    { x: 300, y: 180 },
    ...
    <div className="relative aspect-square w-full">
      <div className="absolute inset-8 rounded-2xl border border-border bg-card shadow-sm">
        <div className="flex items-center gap-1.5 border-b border-border px-4 py-3">
          ...
          <span className="h-2 w-2 rounded-full bg-muted/40" />
          <span className="h-2 w-2 rounded-full bg-muted/40" />
          <span className="ml-auto mono-label text-[10px] text-muted">message.tsx</span>
        </div>
...
/* ------------------------------------------------------------------ */
/* 2. Contact panel (left)                                             */
/* ------------------------------------------------------------------ */

function ContactPanel({ copied, onCopy }: { copied: boolean; onCopy: () => void }) {
  return (
    <Reveal className="space-y-5">
      <div className="rounded-2xl border border-border bg-card p-6">
        <p className="mono-label text-muted mb-2">Email</p>
      ...
      <div className="divide-y divide-border rounded-2xl border border-border bg-card">
        ...
        >
          <span className="flex items-center gap-2.5">
            <Linkedin size={16} className="text-muted transition-colors group-hover:text-accent" />
            LinkedIn
          </span>
          <ArrowUpRight size={15} className="text-muted transition-all duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-accent" />
        </a>
        <a
          href={profile.github}
          target="_blank"
        ...
        >
          <span className="flex items-center gap-2.5">
            <Github size={16} className="text-muted transition-colors group-hover:text-accent" />
            GitHub
          </span>
          <ArrowUpRight size={15} className="text-muted transition-all duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-accent" />
        </a>
...
function ProcessGraphic() {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-6">
      <p className="mono-label text-muted mb-5">How it starts</p>
        ...
        {steps.map((step, i) => (
          <div key={step} className="flex items-center">
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="mono-label text-[9px] text-muted">{step}</span>
            </div>
            {i < steps.length - 1 && (
              <motion.span
...
function FormField({
  ...
  const borderClass = hasError ? 'border-destructive' : 'border-border'
        ...
        <AnimatePresence mode="wait">
          {status === 'success' ? (
            ...
            >
              ...
              <p className="mt-6 text-xl font-medium">Message received.</p>
              <p className="mt-2 text-sm text-muted">I&rsquo;ll get back to you soon.</p>
              <p className="mt-4 max-w-xs text-xs text-muted/70">
                Note: no delivery service is connected yet, so this is a placeholder confirmation.
              </p>
              <button
                onClick={onReset}
                className="mt-6 text-sm font-medium text-accent hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/60 rounded-sm"
              >
            ...
            >
              <div className="grid gap-5 sm:grid-cols-2">
                <FormField id="name" label="Name" required maxLength={100} hasError={fieldErrors.name} />
                <FormField id="email" label="Email" type="email" required maxLength={150} hasError={fieldErrors.email} />
              </div>
              <FormField id="subject" label="Subject" maxLength={150} />
              <FormField id="message" label="Message" required maxLength={2000} rows={5} hasError={fieldErrors.message} />
